...
export interface WatermarkPosition {
  /** Decalage horizontal en pourcentage, -50 a 50. */
  readonly offsetX: number;
  /** Decalage vertical en pourcentage, -50 a 50. */
  readonly offsetY: number;
  /** Rotation en degres. */
  readonly angle: number;
  /** Index du pas courant, utile pour les tests et l'animation. */
  readonly step: number;
}

const POSITION_CYCLE: readonly { x: number; y: number; a: number }[] = [
  { x: 0, y: 0, a: -30 },
  { x: 8, y: -6, a: -27 },
  { x: -7, y: 5, a: -33 },
  { x: 5, y: 8, a: -29 },
...
/**
 * Position du filigrane visible a un instant donne.
 * Par defaut le motif change toutes les 30 secondes (brief section 4.3).
 */
export function watermarkPositionAt(nowMs: number, periodMs = 30_000): WatermarkPosition {
  const step = Math.floor(nowMs / periodMs);
  const slot = POSITION_CYCLE[((step % POSITION_CYCLE.length) + POSITION_CYCLE.length) % POSITION_CYCLE.length];
  const safe = slot ?? { x: 0, y: 0, a: -30 };
  return { offsetX: safe.x, offsetY: safe.y, angle: safe.a, step };
}

/** Libelle affiche dans le filigrane visible. */
export function watermarkLabel(email: string, phone: string, at: Date): string {
  const stamp = at.toISOString().slice(0, 16).replace('T', ' ');
  return `${email} · ${phone} · ${stamp} UTC`;
}
